 ...
 * Build: http://modernizr.com/download/#-fontface-backgroundsize-multiplebgs-opacity-rgba-generatedcontent-csstransitions-input-inputtypes-touch-shiv-mq-cssclasses-teststyles-testprop-testallprops-prefixes-domprefixes-load
 */
...
function updateMiniBag(calcshipping, callback){
    $.ajax({
        type: 'POST',
        data: $.param({
        ...
        url: '/r/ajax/GetBagInfo.jsp',
        success: function(data){
            var obj = parseRtn(data);
            if (obj.success){

                var itemCount = parseInt(obj.msg1, 10);
                $('#cart_item_count').text(itemCount);
                var additionalMessage = $('#my_bag_addl_msg');
                var additionalItemCount = $('#my_bag_addl_item_count');
                if (itemCount > 3){
                    additionalItemCount.text(itemCount - 3);
                    additionalMessage.show();
                } else {
                    additionalItemCount.text(0);
                    additionalMessage.hide();
                }
                if (obj.msg2){
                    $('#my_bag_total').html(obj.msg2);
                }

                var isEmpty = true;
                var counter = 0;
                var products = new Array(obj.msg7, obj.msg8, obj.msg9);
                for (var i=0; i<products.length; i++){
                    var product = products[i];
                    var details = product.split('|');
                    var row = $('#minicart_item' + (i));
                    var code = details[0] ? details[0] : '';
                    var size = details[1] ? details[1] : '';
                    var imgSource = details[6] ? details[6] : 'i/dot.png';
                    var imgTarget = details[3] ? details[3] : '';
                    var brandTarget = details[5] ? details[5] : '';
                    var brandName = details[4] ? details[4] : '';
                    var name = details[2] ? details[2] : '';
                    var price = details[7] ? details[7] : '';
                    var origPrice = details[8] && details[8] != null && details[8] != 'null' ? details[8] : null;
                    var itemMessage = details[9] && details[9] != null && details[9] != 'null' ? details[9] : null;
                    row.removeClass().addClass('rev_minicart_item ' + code + ' ' + size.replace(/[ /()]/g, '_'));
                    $('.product_image img', row).attr('src', imgSource);
                    $('.product_image a', row).attr('href', imgTarget);
                    $('.designer_brand a', row).attr('href', brandTarget).text(brandName);
                    $('.product_name', row).html(name);
                    $('.price, .discount_price', row).html(price);
                    if (origPrice != null){
                        $('.original_price', row).html(origPrice).show();
                        $('.price', row).text('').hide();
                    } else {
                        $('.original_price, .discount_price', row).text('').hide();
                        $('.price', row).show();
                    }

                    if (itemMessage != null)
                        $('.item_message', row).html(itemMessage).show();
                    else
                        $('.item_message', row).text('').hide();
                ...
                if (isEmpty){
                    $('#minicart_item_empty').show();
                } else {
                    $('#minicart_item_empty').hide();
                }
            }
...
function customerCareContentAdjustments(cc) { }

function showButtonWorking(element, text){
    var text = $('<div />').html(text).text();
    var button = $(element);
    var originalText = button.val();
    button.prop('disabled', true).addClass('working_button');

    var intervalID = setInterval(function(){
        var buttonText = button.val();
        if (button.hasClass('working_button')){
            if (buttonText === text)
                button.val(text + '.');
            ...
            showButtonDone(intervalID, element, originalText);
        }
    }, 150);
    setTimeout(function(){showButtonDone(intervalID, element, originalText);}, 5000);
    return intervalID;
}

function showButtonWorkingInfinite(element, text){
    var text = $('<div />').html(text).text();
    var button = $(element);
    var originalText = button.val();
    button.prop('disabled', true).addClass('working_button');
...
function showButtonDone(intervalID, element, text){
    clearInterval(intervalID);
    var button = $(element);
    if (button.hasClass('working_button') && text)
        button.val($('<div />').html(text).text());
    button.prop('disabled', false).removeClass('working_button');
}
